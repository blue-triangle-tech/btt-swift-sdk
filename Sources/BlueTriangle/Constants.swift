...
enum Constants {
    ...
    static func configEndPoint(for siteId: String) -> URL? {
        guard let url = URL(string: "https://\(siteId).btttag.com/config.php?siteID=\(siteId)&os=\(Constants.os)&osver=\(Device.current.osVersion)&app=\(Device.current.appVersion)&sdk=\(Device.current.sdkVersion)") else {
            return nil
        }
        return url
    }
