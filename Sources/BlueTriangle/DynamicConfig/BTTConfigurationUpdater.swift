        ...
        configFetcher.fetch {  fetchedConfig, error  in
            ...
            if let config = fetchedConfig{
                ...
                do{
                    ...
                    self.logger?.info("BlueTriangle:BTTConfigurationUpdater - Remote config fetched successfully \(config.networkSampleRateSDK ?? 0) - sdk \(config.enableAllTracking ?? true ? "true" : "false") - grouping : \(config.enableGrouping ?? false ? "true" : "false") , screenTracking: \(config.enableScreenTracking ?? false ? "true" : "false") ")
                }
                catch{
